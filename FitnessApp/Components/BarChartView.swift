...
//      -
        ...
        linePath.addLine(to: CGPoint(x: bounds.width, y: bounds.width - margin))
//      -
        linePath.stroke()
        ...
        verticalTopLabel.draw(at: CGPoint(x: 0, y: margin), withAttributes: labelAttributes as [NSAttributedString.Key : Any])
        
        let midDataValue = Int(highestDataValue / 2)
        
        let verticalMidLabel = String(midDataValue) as NSString
        verticalMidLabel.draw(at: CGPoint(x: 0, y: CGFloat(graphMidPoint) + margin), withAttributes: labelAttributes as [NSAttributedString.Key : Any])
        
        let verticalBottomLabel = String(0) as NSString
        verticalBottomLabel.draw(at: CGPoint(x: 0, y: bounds.height - margin), withAttributes: labelAttributes as [NSAttributedString.Key : Any])
        ...
        for i in 0...barGraphData.count - 1 {
            ...
            label.draw(at: textPos, withAttributes: labelAttributes as [NSAttributedString.Key : Any])
        }
    }
